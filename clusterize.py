...
w = open(sys.argv[1], 'w')
tree = etree.parse('result/xml/strict_out.xml')
root = tree.getroot()
...
for child in root.iter('StrictName'):
    if child[1].attrib['val'] == 'NONE' and child[2].attrib['val'] == 'NONE' and child[3].attrib['val'] == 'NONE':
        ...
        PER[c] = {'NameNotNorm': child[0].attrib['val'], 'Firstname': child[1].attrib['val'],
                  'Lastname': child[2].attrib['val'], 'Patronymic': child[3].attrib['val'],
                  'Gender': child[5].attrib['val'], 'Descr': child[6].attrib['val']}
...
for i in range(1):
    ...
    for i in PER:
        if i not in DOTS:
            w.write('\nPER\n')
            if PER[i]['Firstname'] == 'NONE' and PER[i]['Patronymic'] == 'NONE' and len(
                    PER[i]['Lastname'].split()) == 1:
