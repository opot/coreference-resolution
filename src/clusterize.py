...
def predict(mentions):
    ...
    for current in range(len(mentions)):
        ...
        probabilities = [
            sum(relations[entity][mention][best_relation[entity][0]]
                for mention in range(len(entities[entity])))
            /
            sum(sum(relations[entity2][mention][best_relation[entity][0]]
                    for mention in range(len(entities[entity2])))
...
if __name__ == "__main__":
    ...
    for entity in entities:
        ...
        if "fname" in obj:
            print("Firstname", obj["fname"])
        if "patr" in obj:
            print("Patronimyc", obj["patr"])
        if "lname" in obj:
            print("Lastname", obj["lname"])
        print()
